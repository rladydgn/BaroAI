...
def get_memory_info() -> str:
    ...
    info += "{:<20}: ".format("total") + str(round(memory.total / 1024 ** 3, 3)) + " GB \n"
    info += "{:<20}: ".format("available") + str(round(memory.available / 1024 ** 3, 3)) + " GB \n"
    info += "{:<20}: ".format("using") + str(round(memory.used / memory.total * 100, 3)) + " % \n\n"

    memory_info.extend([round(memory.total / 1024 ** 3, 3), round(memory.available / 1024 ** 3, 3),
                        round(memory.used / memory.total * 100, 3)])
...
def get_disk_info() -> str:
    ...
    for d in disk:
        ...
        info += "{:<20}".format("using_disk") + str(round(int(d[2]) / 1024 ** 2, 3)) + " GB \n"
        info += "{:<20}".format("using_disk_percent") + str(round(int(d[2]) / int(d[1]) * 100, 3)) + " % \n\n"

        disk_info.extend([d[0], round(int(d[2]) / 1024 ** 2, 3), round(int(d[2]) / int(d[1]) * 100, 3)])

        disks_info.append(disk_info)

    info += "{:<20}".format("total_disk") + str(round(disk_total / 1024 ** 2, 3)) + " GB \n"
    info += "{:<20}".format("total_usable_disk") + str(round(disk_usable_total / 1024 ** 2, 3)) + " GB \n\n"

    disk_total_info.extend([round(disk_total / 1024 ** 2, 3), round(disk_usable_total / 1024 ** 2, 3)])
...
def get_gpu_info() -> str:
    ...
    for i in range(number_of_gpu):
        ...
        using_percent: float = round(int(gpu_using_memories[i * 2].split(':')[-1].split()[0]) / int(
            gpu_memories[i * 2].split(':')[-1].split()[0]) * 100, 3)
    ...
    info += "{:<20}: ".format("total_using_percent") + str(
        round(gpu_total_using_memory / gpu_total_memory * 100, 3)) + " %\n"

    gpu_total_info.extend([number_of_gpu, gpu_total_memory, gpu_total_using_memory,
                           round(gpu_total_using_memory / gpu_total_memory * 100, 3)])
